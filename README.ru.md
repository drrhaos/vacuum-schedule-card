...
Кастомная карточка для Home Assistant, позволяющая создавать и управлять расписанием уборки для роботов-пылесосов (включая Dreame, Xiaomi M30S, Xiaomi D103CN и другие) с использованием интеграции [dreame-vacuum](https://github.com/Tasshack/dreame-vacuum) или [xiaomi_miot](https://www.home-assistant.io/integrations/xiaomi_miot/).
...
integration: dreame_vacuum  # Обязательно: dreame_vacuum, xiaomi_miot или standard
...
**Типы интеграций:**
- `dreame_vacuum` - Для пылесосов Dreame и некоторых моделей Xiaomi, использующих интеграцию dreame-vacuum
- `xiaomi_miot` - Для устройств Xiaomi MIOT (например, D103CN), использующих интеграцию xiaomi_miot
- `standard` - Для стандартных интеграций пылесосов (использует базовый сервис vacuum.start)

### Параметры конфигурации

- **`entity`** (обязательно) - Entity ID пылесоса (например, `vacuum.xiaomi_m30s`)
- **`integration`** (обязательно) - Тип интеграции пылесоса: `dreame_vacuum`, `xiaomi_miot` или `standard`
  - Используйте `dreame_vacuum` для пылесосов Dreame и некоторых моделей Xiaomi, использующих интеграцию dreame-vacuum
  - Используйте `xiaomi_miot` для устройств Xiaomi MIOT (например, D103CN), использующих интеграцию xiaomi_miot
  - Используйте `standard` для стандартных интеграций пылесосов (использует базовый сервис vacuum.start)
...
**Для интеграции xiaomi_miot:**
- ID комнат обычно доступны в атрибутах entity пылесоса
- Проверьте атрибуты `segments` или `room_list` в Developer Tools → States
- Для некоторых моделей может потребоваться использование приложения Mi Home для просмотра соответствий комнат
...
- **Действие**: Вызов сервиса `dreame_vacuum.vacuum_clean_segment` или `xiaomi_miot.vacuum_clean_segment` с указанием комнат (в зависимости от используемой интеграции)
...
- Одна из следующих интеграций:
  - Интеграция [dreame-vacuum](https://github.com/Tasshack/dreame-vacuum) от Tasshack (для Dreame и некоторых моделей Xiaomi)
  - Интеграция [xiaomi_miot](https://www.home-assistant.io/integrations/xiaomi_miot/) (для устройств Xiaomi MIOT, таких как D103CN)
- Поддерживаемые роботы-пылесосы:
  - Роботы-пылесосы Dreame
  - Xiaomi M30S (через dreame-vacuum)
  - Xiaomi D103CN / M30S (через xiaomi_miot)
...
│   │   ├── event-subscription.ts # Утилиты подписки на события
...
│   │   └── automation.ts        # Типы автоматизаций
│   └── assets/                  # Ресурсы
...
└── README.ru.md                 # Документация (русская)
```
