        ...
        <div class="control-row rooms-row">
          ${s.length>0?H`
            ${(()=>{const t=this._isCleaning(),e=t;return H`
              ...
              `})()}
            ${s.map(t=>{const e=this._isCleaning(),o=this._currentCleaningRooms.includes(t.id),r=this.selectedRooms.includes(t.id);return H`
                <ha-card 
                  class="room-button ${o||r&&!e?"pressed":""} ${e?"disabled":""}"
                  @click=${e?void 0:()=>this._toggleRoom(t.id)}
                  title="${t.name}${this.showRoomIds?` (ID: ${t.id})`:""}${o?" (убирается)":""}"
                >
                  <div class="button-content">
